...
class ArchiveTweet:
    ...
    def archive_account(self, query: str):
        ids = []
        hashtags = []
        for each_status in tweepy.Cursor(self.api.user_timeline, screen_name = query, count = 1000).items():
            ...
            for each_hashtag in each_status.entities.get('hashtags'):
                ...
                if self.debug:
                    print(each_status.user.screen_name, each_status.text.strip('\n'), end='\n\n')
        return ids, hashtags

    def archive_hashtag(self, query: str):
        ids = []
        for each_tweet in got.manager.TweetManager.getTweets(got.manager.TweetCriteria().setQuerySearch(query)):
            ids.append(str(each_tweet.id)+'\n')
        for each_status in tweepy.Cursor(self.api.search, q=query, count=100).items():
            ids.append(each_status.id_str + '\n')
        return ids
